...
void __fastcall TMainForm::BBuildClick(TObject *Sender)
{
 ...
 MainForm->Memo1->Lines->Add("Функция:");
...
void TDrFunction::build()
{
 ...
 if (!this->showing)
   {MainForm->Memo1->Lines->Add("График не показан");
    return;
   }
 int lp=1,llp=1;
...
#define putpixel(_X,_Y) bmp->Canvas->Pixels[_X][_Y]=bmp->Canvas->Pen->Color
 ...
 if (solver->geterr()!=E_NO)
   {MainForm->Memo1->Lines->Add(AnsiString("Фатальная ошибка: "));
...
{int i;
 ...
 for (i=0;i<fns;i++,MainForm->Memo1->Lines->Add(""))
   {MainForm->Memo1->Lines->Add("Функция ("+AnsiString(i+1)+" из "+AnsiString(fns)+"):");
    functions[i]->build();
    Application->ProcessMessages();
   }
 ShowBitmap();
...
{if (changed)
 switch
   (MessageBox(0,"Сохранить файл перед закрытием?",
               "Завершение работы...",MB_YESNOCANCEL)
   )
