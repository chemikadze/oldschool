...
begin
  ...
  begin // ->                // иначе нажали на кнопку "увелич"
    for i:=queryCount downto 2 do  // соритировка пузырькм по убыванию
      for j:=1 to i-1 do
        if query[j]>query[j+1] then
        begin
          b := query[j];
  ...
  // движемся в противоположную первоначальной сторону
  for i:=startI-1 downto 1 do
  begin
    time := time + timeMove*abs(position-query[i]);
    position := query[i];
    StringGrid2.Cells[outPos, 0] := IntToStr(position);
