...
begin
  ...
  Image1.Canvas.Pen.Color := clBlue;   // устанавливаем цвет фигуры
  Image1.Canvas.Brush.Color := clBlue;
  // чертим фигуру

  // перевод из обычных координат в координаты канвы
  ...
  Image1.Canvas.LineTo(Round((Image1.Width-20)/8 * 0 +10), Image1.Height - Round((Image1.Height-20)/8 * 8 ) - 10);
  Image1.Canvas.LineTo(Round((Image1.Width-20)/8 * 0 +10), Image1.Height - Round((Image1.Height-20)/8 * 2 ) - 10);
  Image1.Canvas.FloodFill(Round((Image1.Width-20)/8 * 3 +10), Image1.Height - Round((Image1.Height-20)/8 * 3 ) - 10,
                          clWhite, fsSurface);
  // рисуем точку
  ...
  // условие
  if (x>=0) and (x<=8) and (y>=0) and
  (
    (y<=8-x) and
    not ((x>=0) and (x<2) and (y>=0) and (y<2))
  )
  then
    // извинити за сексизм
