...
begin
  ...
  begin
    line := Memo1.Lines[i];
    p:=1-length(rpl); // при подстановке в p+length(rpl) получим 1
    while (true) do // "бесконечный" цикл
    begin
      p:=posex(fnd, line, p+length(rpl)); // находим позицию подстроки начиная с позиции после последней замены
      if (p<=0) then              // если строка не найдена
