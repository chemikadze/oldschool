...
begin
  ...
  begin
    ...
    begin
      c:= Memo1.Lines[i][j];
      b := false;        // индикатор замены
      // символ - буква....
      if ((c>='а') and (c<='я')) or ((c>='А') and (c<='Я')) then
      begin
        // делаем букву маленькой чтобы проверить ее наличие в списке гласных...
        if (c>='А') and (c<='Я') then
          c := chr(ord(c) + (ord('а')-ord('А')));
        for k:=1 to length(glas) do
          if (glas[k] = c) then // буква - гласная
          begin
            line := line + IntToStr(k); // добавляем к буферу
            b := true;
          end;
      end;
      if (not b) then // если не заменяли символ (это не гласная буква)
