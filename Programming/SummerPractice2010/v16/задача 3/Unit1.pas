...
var line: string;
    i, j: integer;
// конвертируем построчно, если спросят почему так - ответь чтобы не получить одну
// огромную строку
begin
  for i:=0 to Memo1.Lines.Count do
  begin
    // обнуляем измененную строку
    line := '';
    // формируем измененную строку проходя по всем символам
    for j:=1 to length(Memo1.Lines[i]) do
      // добавляем к строке код символа и ';'
      line := line + IntToStr( Ord(Memo1.Lines[i][j]) ) + ';';
    // заменяем строку
    Memo1.Lines[i] := line;
  end;
end;
