...
// получаем из больших букв маленькие
function lowercase(c: char):char;
begin
  if (c>='А') and (c<='Я') then
    result := chr(ord(c) + ord('а') - ord('А'))
  else
    result := c;
end;
...
begin
  ...
  begin
    wn := 1;
    j:=1;
    while (j <= length(Memo1.Lines[i])) do // обрабатываем строку
    begin
      if ((Memo1.Lines[i][j] <> ' '))  // считаем что слово - это все что не пробел
      then
      begin
        if (pos(lowercase(Memo1.Lines[i][j]), 'аеёиоуыэюя')=0) then     // если слово не начинается с гласной
        begin
          while (j <= length(Memo1.Lines[i])) and (Memo1.Lines[i][j] <> ' ') do // ищем его конец
          begin
            line := line + Memo1.Lines[i][j]; // добавляем к измененной строке символ
            inc(j);
          end;
        end
        else
        begin // четные пропускаем
      ...
      begin
        line := line + Memo1.Lines[i][j]; // добавляем пробел
        inc(j);
      end;
    end;
    Memo1.Lines[i] := line;  // заменяем строку измененной
  end;
end;
